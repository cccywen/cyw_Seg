...
class Initialization(nn.Module):
    def __init__(self, output_num_filters):
        super().__init__()
        self.convolution = nn.Conv2d(1, output_num_filters, (1, 1), bias=True)
...
class ResNet(nn.Module):
    def \
            __init__(self, num_filters, dilations):
        super().__init__()

        self.common_instance_normalization = nn.InstanceNorm2d(
            num_filters, affine=True
        )
        self.first_convolutions = nn.ModuleList(
            [
                nn.Conv2d(
                    num_filters,
                    num_filters // 2,
        ...
        self.instance_normalizations = nn.ModuleList(
            [nn.InstanceNorm2d(num_filters // 2, affine=True) for _ in dilations]
        )
        self.second_convolutions = nn.ModuleList(
            [
                nn.Conv2d(
                    num_filters // 2,
                    num_filters,
...
class Combining(nn.Module):
    def __init__(self, first_num_filters, second_num_filters, output_num_filters):
        ...
        self.instance_normalization = nn.InstanceNorm2d(
...
class ResUNetA(nn.Module):
    def __init__(self, nclass):
        ...
        self.down_res_nets = nn.ModuleList(
            [
        ...
        self.up_res_nets = nn.ModuleList(
            [
                ResNet(stage_num_filters, stage_dilations)
                for stage_num_filters, stage_dilations in zip(
        ...
        self.downscalings = nn.ModuleList(
            [
                Downscaling(
...
def get_resuneta(dataset='citys'):
